# InstaHost UI design system

This document defines InstaHost's UI design system for its Livewire + Blade +
...
The interface is compact and product-focused, modeled on the Heroku
Dashboard (see `Documents/herokudesign.md`):

- cold silver paper canvas (`#F7F8FB`) instead of large bordered boxes;
- 13px UI typography and 32px controls with 4px radii;
- hairline `#E3E7EF` rings instead of heavy borders;
- full-width data tables for dense collections;
- outline Reicon glyphs through `<x-reicon>`;
- Heroku product purple (`#79589F`) as the brand accent in light mode;
- a lighter purple (`#C29EF1`) as the accent in dark mode;
- primary actions use a 135° purple gradient (`#8964B4` → `#6A4791`);
  secondary actions are purple outline buttons;
- resource tabs use an underline (`#A997BF`), not a filled pill;
...
|---|---|---|---|
| `--coollabs-canvas` | `#F7F8FB` | `#323B49` | page canvas |
| `--coollabs-elevated` | white | `#3D4858` | shells and card headers |
| `--coollabs-base` | white | `#3D4858` | nested card bodies |
| `--coollabs-recessed` | `#EEF1F6` | `#475366` | inputs and listboxes |
| `--coollabs-fill` | `#E3E7EF` | `#56667D` | dividers and passive fills |
| `--coollabs-line` | `#CFD7E6` | `#62738D` | control borders |
| `--coollabs-hairline` | `#E3E7EF` | `#475366` | shell rings |
| `--coollabs-subtle` | `#62738D` | `#CFD7E6` | labels and muted titles |

Accent behavior is theme-aware:

- **Light mode:** Heroku product purple (`#79589F` / `coollabs`) for active
  controls, focus, primary actions, and navigation accents.
- **Dark mode:** Heroku light purple (`#C29EF1`) for the same states so the
  accent stays readable on slate.

Do not hard-code blue focus rings. Primary highlighted actions use the Heroku
dashboard gradient rather than a flat fill:

```css
background: linear-gradient(135deg, #8964B4, #6A4791);
```

Default outline buttons use a 1px `#79589F` border, 13px semibold label, and
4px radius. The filled top-level tab treatment is an underline, not a tinted
pill.
...
- Main sidebar groups are compact, use outline Reicons, and keep a 32px row
  height.
- Active sidebar rows are rounded (`rounded`) with a lavender selected fill
  (`#F7F3FB`) and purple text (`#79589F`). Hover rows use the same radius.
...
layer-2 bar on another. Active tabs are an underline in brand purple:

- `#79589F` text and `#A997BF` underline in light mode;
- `#C29EF1` text in dark mode;
...
All normal controls are 32px high with a 4px radius.
