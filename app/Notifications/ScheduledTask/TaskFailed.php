...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("InstaHost: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = "InstaHost: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in InstaHost',
                'url' => (string) $this->url,
            ];
        }
