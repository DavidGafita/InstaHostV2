...
class DockerCleanupFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("InstaHost: [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}");
    ...
    public function toDiscord(): DiscordMessage
    {
        return new DiscordMessage(
            title: ':cross_mark: InstaHost: [ACTION REQUIRED] Docker cleanup job failed on '.$this->server->name,
    ...
    {
        return [
            'message' => "InstaHost: [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}!\n\n{$this->message}",
        ];
    }
    ...
    public function toSlack(): SlackMessage
    {
        return new SlackMessage(
            title: 'InstaHost: [ACTION REQUIRED] Docker cleanup job failed',
