    ...
    @auth
        <div x-data="{
            ...
            <header
                ...
                class="hidden lg:flex fixed top-0 inset-x-0 z-50 h-12 items-center bg-white dark:bg-panel/95 backdrop-blur-none">
                {{-- Brand (width tracks sidebar) --}}
                <div class="flex items-center gap-2 h-full shrink-0 border-r border-[#e3e7ef] dark:border-white/[0.06] transition-[width] duration-200"
                    :class="collapsed ? 'w-16 justify-center px-0' : 'w-56 px-4'">
                    <div class="flex shrink-0 items-baseline gap-1.5 min-w-0">
                        <a href="/" {{ wireNavigate() }} title="{{ brand_full_name() }}"
                            class="flex items-center hover:opacity-80 transition-opacity">
                            <img x-show="collapsed" x-cloak src="/coolify-logo.svg" alt="InstaHost"
                                class="size-5" />
                            <span x-show="!collapsed" class="flex min-w-0 flex-col leading-none">
                                <span class="text-[15px] font-semibold tracking-[0.04em] uppercase text-[#79589f] dark:text-white">InstaHost</span>
                                <span class="mt-0.5 text-[8.5px] font-medium tracking-[0.08em] uppercase text-[#62738d] dark:text-fg-faint">{{ brand_tagline() }}</span>
                            </span>
                        </a>
                ...
                <div
                    class="flex h-full items-center gap-0.5 min-w-0 flex-1 border-b border-[#e3e7ef] pl-3 pr-4 dark:border-white/[0.06]">
                ...
                <div class="fixed inset-y-0 right-0 flex h-full">
                    <div
                        ...
                        <div data-mobile-sidebar-brand
                            ...
                            <a href="/" {{ wireNavigate() }} title="{{ brand_full_name() }}"
                                class="flex min-w-0 flex-col leading-none transition-opacity hover:opacity-80">
                                <span class="text-[15px] font-semibold tracking-[0.04em] uppercase text-[#79589f] dark:text-white">InstaHost</span>
                                <span class="mt-0.5 text-[8.5px] font-medium tracking-[0.08em] uppercase text-[#62738d] dark:text-fg-faint">{{ brand_tagline() }}</span>
                            </a>
            ...
            <div
                ...
                <div class="flex min-w-0 flex-1 items-center gap-2.5">
                    <a href="/"
                        ...
                        <img src="/coolify-logo.svg" alt="InstaHost" class="w-[18px] h-[18px]" />
                    </a>
