        ...
        <x-application.settings-section id="advanced-build-section" title="Build"
            ...
            <div class="grid w-full gap-4 sm:grid-cols-2">
                ...
                <x-forms.listbox id="injectBuildArgsToDockerfile" label="Build arguments" onChange="instantSave"
                    helper="When injected automatically, InstaHost adds ARG statements to your Dockerfile for build-time variables. Manage them manually to preserve Docker build cache."
                    :options="[
            ...
            <x-application.settings-section id="advanced-compose-section" title="Docker compose"
                ...
                <div class="grid w-full gap-4 sm:grid-cols-2">
                    <x-forms.listbox id="isRawComposeDeploymentEnabled" label="Compose deployment" onChange="instantSave"
                        helper="WARNING: Advanced use cases only. In raw mode your compose file is deployed as-is. Nothing is modified by InstaHost and you need to configure the proxy parts. More info in the <a class='underline dark:text-white' href='https://coolify.io/docs/knowledge-base/docker/compose#raw-docker-compose-deployment'>documentation</a>."
                        :options="[
                            ['value' => false, 'label' => 'Managed by InstaHost'],
                    ...
                    <x-forms.listbox id="isConnectToDockerNetworkEnabled" label="Predefined network" onChange="instantSave"
                        helper="By default a compose resource only gets its own internal network. Connecting to an InstaHost predefined network may require different internal DNS names. More info <a class='underline dark:text-white' target='_blank' href='https://coolify.io/docs/knowledge-base/docker/compose#connect-to-predefined-networks'>here</a>."
                        :options="[
        ...
        <x-application.settings-section id="advanced-proxy-section" title="Proxy"
            ...
            @else
                <x-empty size="sm" title="Proxy behavior is managed through labels"
                    description="Container labels are managed manually for this application. Switch label management back to InstaHost to configure the proxy here."
                    icon-name="globe">
                    <x-slot:contents>
                        <a class="button"
        ...
        <x-application.settings-section id="advanced-operations-section" title="Operations"
            ...
            <div class="grid w-full gap-4 lg:grid-cols-2">
                ...
                <x-forms.input type="number" min="0" id="maxRestartCount" label="Max restart count"
                    ...
                    helper="Maximum number of crash restarts before InstaHost automatically stops the application and sends a notification. Saved automatically. Set to 0 to disable the limit."
                    canGate="update" :canResource="$application" />
            </div>
        </x-application.settings-section>
