    ...
    @if ($server->proxyType())
                                ...
                                @if ($proxySettings)
                                    <x-modal-confirmation title="Reset Proxy Configuration?"
                                        ...
                                        submitAction="resetProxyConfiguration" :actions="[
                                            'Reset the proxy configuration to InstaHost defaults.',
        ...
        @elseif($selectedProxy === 'NONE')
            <x-application.settings-section title="Custom proxy"
                helper="InstaHost will not manage a reverse proxy for this server.">
                <x-slot:actions>
                    @can('update', $server)
                ...
                <x-callout type="info" title="Custom proxy selected">
                    Configure and operate the proxy outside InstaHost.
                </x-callout>
            </x-application.settings-section>
        @else
            <x-application.settings-section title="Proxy configuration"
                ...
                @can('update', $server)
                    <div class="grid gap-3 lg:grid-cols-3">
                        @foreach ([
                            ['value' => 'NONE', 'title' => 'Custom', 'description' => 'Manage the proxy outside InstaHost.'],
                            ['value' => 'TRAEFIK', 'title' => 'Traefik', 'description' => 'Use the default InstaHost proxy.'],
                            ['value' => 'CADDY', 'title' => 'Caddy', 'description' => 'Use the InstaHost Caddy integration.'],
                        ] as $proxyOption)
