<div>
    <x-slot:title>
        {{ data_get_str($server, 'name')->limit(10) }} > Server Resources | InstaHost
    </x-slot>
    ...
    <div class="server-settings-workspace application-settings-workspace mt-4 grid w-full max-w-none min-w-0 gap-8 lg:mt-0 xl:grid-cols-[210px_minmax(0,1fr)] xl:gap-8">
        ...
        <x-application.settings-section id="server-resources-section" title="Resources"
            helper="Review InstaHost-managed resources and other Docker containers running on this server."
            flush>
            <x-slot:actions>
                ...
                @else
                    <div class="p-6">
                        <x-empty size="sm" title="No unmanaged containers"
                            description="All detected Docker containers are managed by InstaHost."
                            icon-name="servers" />
                    </div>
                @endif
            @endif
