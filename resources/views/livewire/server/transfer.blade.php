<div>
    <x-slot:title>
        {{ data_get_str($server, 'name')->limit(10) }} > Transfer | InstaHost
    </x-slot>
        ...
        <div class="application-settings-form flex w-full flex-col gap-6">
            @if ($this->isLocalhost)
                <x-application.settings-section id="server-transfer-section" title="Transfer server (Dev)"
                    helper="Move this server’s control-plane config to another InstaHost instance (same physical host).">
                    <x-callout type="warning" title="Localhost cannot be transferred">
                        The InstaHost host (localhost) cannot be transferred between instances.
                    </x-callout>
                    ...
                    <div class="flex flex-col gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs leading-5 text-neutral-500 dark:text-fg-dim">
                                Enter the target InstaHost URL and an API token from that instance (root recommended).
                        ...
                        <div class="flex flex-col gap-3 md:max-w-xl">
                            <x-forms.input id="targetUrl" label="Target instance URL" required
                                ...
                                helper="Base URL of the other InstaHost instance (no trailing path)." />
