            ...
            <x-application.settings-section title="General">
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    @if (str_starts_with(strtolower($fqdn ?? ''), 'https://'))
                        <div>
                            <x-forms.listbox canGate="update" :canResource="$settings"
                                ...
                                onChange="submit"
                                helper="Disable only when Cloudflare Tunnel or another proxy connects to InstaHost over HTTP. Keep enabled when Cloudflare uses Full or Full (Strict) SSL."
                                :options="[
                    ...
                    <x-forms.input canGate="update" :canResource="$settings" id="instance_name" label="Name"
                        placeholder="InstaHost" helper="Custom name for this InstaHost instance." />
            ...
            <x-application.settings-section title="Network addresses">
                <div class="grid gap-4 lg:grid-cols-2">
                    <x-forms.input canGate="update" :canResource="$settings" id="public_ipv4" type="password"
                        ...
                        helper="Set this when InstaHost cannot detect the correct public IPv4 address."
                    ...
                    <x-forms.input canGate="update" :canResource="$settings" id="public_ipv6" type="password"
                        ...
                        helper="Set this when InstaHost cannot detect the correct public IPv6 address."
    ...
    <x-domain-conflict-modal :conflicts="$domainConflicts" :showModal="$showDomainConflictModal"
        ...
        <x-slot:consequences>
            <ul class="mt-2 ml-4 list-disc">
                <li>The InstaHost instance domain will conflict with existing resources.</li>
                <li>SSL certificates might not work correctly.</li>
                <li>Routing behavior will be unpredictable.</li>
                <li>You may not be able to access the InstaHost dashboard properly.</li>
            </ul>
        </x-slot:consequences>
