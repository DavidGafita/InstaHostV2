<div>
    <x-slot:title>
        {{ $gitlab_app->name ?: 'GitLab App' }} | Sources | InstaHost
    </x-slot>
                    ...
                    @if (! isCloud())
                        <div class="lg:col-span-2 max-w-xs">
                            <x-forms.checkbox canGate="update" :canResource="$gitlab_app" label="System wide"
                                helper="If checked, this GitLab App will be available for everyone in this InstaHost instance."
                                instantSave id="isSystemWide" />
                        </div>
                        @if ($isSystemWide)
                            <div class="lg:col-span-2">
                                <x-callout type="warning" title="Shared with every team">
                                    System-wide GitLab Apps are shared across all teams on this InstaHost instance. Prefer team-specific apps when you need repository isolation.
                                </x-callout>
                            </div>
                        @endif
                    @endif
        ...
        <div class="application-settings-form flex flex-col gap-6"
                    ...
                    @if (! isCloud() || isDev())
                        <div class="mt-4 grid gap-4 lg:grid-cols-2">
                            <div class="lg:col-span-2 text-[12px] text-neutral-500 dark:text-fg-dim">
                                GitLab will redirect back to this InstaHost URL. It must match the Callback URL on your GitLab OAuth Application exactly.
                            </div>
                            <div class="lg:col-span-2 max-w-md">
                                <x-forms.listbox id="use_custom_webhook_endpoint" label="Webhook endpoint"
                                    ...
                                    helper="Use a custom public URL when InstaHost is behind a tunnel or reverse proxy." />
                            </div>
            ...
            @if ($clientId)
                <x-application.settings-section title="Step 3 · Authorize with GitLab"
                    description="Authorize InstaHost with your GitLab instance. The redirect URI must match the Callback URL configured in GitLab.">
