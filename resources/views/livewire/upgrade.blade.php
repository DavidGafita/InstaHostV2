<div @if ($isUpgradeAvailable) title="New version available" @else title="No upgrade available" @endif
    ...
    @elseif ($fullButton)
        <p class="text-sm text-neutral-600 dark:text-fg-dim">InstaHost is up to date.</p>
    @endif
</div>
        ...
        Alpine.data('upgradeModal', (config) => ({
            ...
            simulateUpgrade() {
                ...
                const steps = [
                    ...
                    { step: 3, status: '[DEV] Pulling InstaHost image...' },
            ...
            getReviveStatusMessage(elapsedMinutes, attempts) {
                if (elapsedMinutes === 0) {
                    return `Waiting for InstaHost to come back online... (attempt ${attempts})`;
                } else if (elapsedMinutes < 2) {
                    return `Waiting for InstaHost to come back online... (${elapsedMinutes} minute${elapsedMinutes !== 1 ? 's' : ''} elapsed)`;
                ...
                this.checkUpgradeStatusInterval = setInterval(async () => {
                    try {
                        ...
                        if (!this.serviceDown) {
                            ...
                            this.instanceWentDown = true;
                            this.currentStep = 4;
                            this.currentStatus = 'InstaHost is restarting with the new version...';
                            if (this.checkUpgradeStatusInterval) {
                                clearInterval(this.checkUpgradeStatusInterval);
                                this.checkUpgradeStatusInterval = null;
                            }
